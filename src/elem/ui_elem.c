...
ui_box_t *ui_belem_input(ui_win_t *win, int max_len)
{
    ...
    ui_bhook_add(&input->update, ui_bhook_catch_input);
...
static void ui_navbar_build(ui_box_t* menu, boxtype_e type)
{
	...
	ui_bhook_add(&menu->update, ui_bhook_nohovered);
	ui_bhook_add(&menu->update, ui_bhook_nopressed);
	ui_bhook_add(&menu->on_window_event, widthhandler);
	ui_bhook_add(&menu->on_window_event, heighthandler);
}
...
ui_box_t *ui_option_build(ui_box_t *list, boxtype_e type) 
{
	...
	while(curr) {
		...
		switch(type) {
			...
			ui_bhook_add(&curr->on_window_event, ui_bhook_fullwindow_button);
			case UI_NONE:
			break;
		}
		ui_box_center_layers(curr, NULL);
		ui_bhook_add(&curr->on_click_down, ui_bhook_revealchild);
		ui_bhook_add(&curr->on_key_down, ui_bhook_revealchild);
		curr = curr->next;
		i++;
	}
    return list;
